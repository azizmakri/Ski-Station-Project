pipeline {
    ...
    stages {
        stage('GIT Checkout') { 
            steps {
                script {
                    def branchName = 'aicha'
                    def gitCredentialId = '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'
        ...
        stage('Build Docker image'){
            steps {
                    sh 'sudo docker build -t aichagasri/skistationimage:latest -f DockerFile .'
            }
        }
        ...
        stage('Deploy Image to DockerHub') {
            steps {
                // Pushing the image to Docker Hub
                    sh "sudo docker push aichagasri/skistationimage:latest"
            }
        }
